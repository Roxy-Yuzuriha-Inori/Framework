...
当DDL操作时，获取写锁，不允许其他任何锁 （修改表结构时不能修改数据）
当DML操作时，获取读锁，只允许共同持有读锁的事务 （修改表数据可并发修改，但不能允许修改表结构）

当执行 DDL 时，需要获取 MDL 写锁（独占锁），
会阻塞所有其他事务对该表的读写操作，直到表结构修改完成。

当执行 DML 时，会获取 MDL 读锁（共享锁），
允许多个事务同时访问数据，但不能执行 DDL；
同时会通过行锁（X锁 / S锁）控制具体数据的并发访问。
