...
对于后续的请求，会自动携带cookie去访问服务器<br>
因为浏览器会自动带上 Cookie发送请求，会导致CSRF（通过让已登录状态的用户发送非法请求，而服务器就会接受到Cookie认为合法），而token需要前端手动添加，不会导致CSRF
### 优点
...
public class CookieController {
    ...
    public Result cookie1(HttpServletResponse response){
        ...
        //设置过期时间
        //删除cookie，创建一个同名cookie，然后过期时间为0，请求这个接口
        cookie.setMaxAge(7 * 24 * 60 * 60); // expires in 7 days
        // 仅通过 HTTPS 发送，阻止HTTP请求发送Cookie
        cookie.setSecure(true);
        //阻止浏览器端js通过Document.cookie读取cookie
...
- 用户通过浏览器第一次访问某个服务器，服务器通常自动会将session id 放在cookie返回给用户浏览器（没有cookie也能用session，将id放在url等其他地方）<br>
